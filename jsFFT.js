...
populateFullReal = function(fftlen,realfunc) {
	// create buffer		
...
makeComplex = function(realdata,fftlen) {
...
packRealToHalfComplex = function(realdata,fftlen) {
	// create buffer		
	var buffer = new ArrayBuffer(fftlen*4);
	var data = new Float32Array(buffer);
...
copyComplexArray = function(orig,fftlen) {
	// create complex array
	var buffer = new ArrayBuffer(2*fftlen*4);
	var data = new Float32Array(buffer);
	// copy real data points to complex array
...
jsFFT = function(origdata, nn, sign)			// forward=1 inverse=-1
{
	// copy the data
	var data = copyComplexArray(origdata,nn);
...
jsMagnitude = function(orig, fftlen)
{
	"use strict";
...
jsRealFFT = function(orig, nn, sign)
{
	"use strict";
	var data = copyComplexArray(orig,nn);		// create a copy of data
	var n2 = nn/2;
...
// complex.
unpackToHalfComplexToReal = function(data, nn) {
...
unpackHalfComplexFFT = function(orig, fftlen) {
	// create complex array
	var buffer = new ArrayBuffer(2*fftlen*4);
	var data = new Float32Array(buffer);
